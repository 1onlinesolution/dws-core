...
class BaseLogger {
    ...
    constructor(options, levels = BaseLogger.Levels) {
        ...
        if (options.level !== 'info' && options.level !== 'warn' && options.level !== 'error' && options.level !== 'http' && options.level !== 'debug')
            BaseLogger.throwConfigError();
        this.levels = levels;
    ...
    combinedFormat() {
        if (this.kind === 'console') {
            ...
            format.colorize({ all: true }), format.label({ label: this.label }), format.metadata({ fillExcept: ['message', 'level', 'timestamp', 'label'] }), 
        ...
        format.timestamp({
            format: 'YYYY-MM-DD HH:mm:ss',
        }), format.label({ label: this.label }), format.metadata({ fillExcept: ['message', 'level', 'timestamp', 'label'] }), 
        // Define the format of the message showing the timestamp, the level and the message
        format.printf(printf_1.printf));
    }
